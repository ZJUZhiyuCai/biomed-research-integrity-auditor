...
def normalized_frame_inventory(
    img: Any,
    max_frames: int = DEFAULT_MAX_FRAMES,
) -> tuple[list[tuple[str, Any]], int, bool]:
    """Return normalized frames plus total/truncation metadata.
    ...
    try:
        ...
        return [("", normalized_rgb(img))], 1, False

    n_frames = int(getattr(img, "n_frames", 1) or 1)
    if n_frames <= 1:
        return [("", normalized_rgb(img))], n_frames, False
    ...
    return frames, n_frames, n_frames > len(frames)
...
def iter_normalized_frames(img: Any, max_frames: int = DEFAULT_MAX_FRAMES) -> list[tuple[str, Any]]:
    ...
    frames, _total, _truncated = normalized_frame_inventory(img, max_frames)
    return frames

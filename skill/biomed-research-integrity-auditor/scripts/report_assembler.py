...
def render_coverage(coverage: dict[str, Any] | None) -> list[str]:
    ...
    lines += ["", table([
        ...
        ["Low-information images excluded from global hash matching / 整图哈希排除的低信息图像", str(coverage.get("global_low_information_images_excluded", 0))],
        ["Multi-frame files with unscreened later frames / 存在未筛后续帧的多帧文件", str(len(coverage.get("multi_frame_screening_limits", []) or []))],
        ["Within-stack frame pairs excluded from cross-context reuse / 排除为跨情境复用的同栈帧对", str(coverage.get("intra_stack_pairs_skipped", 0))],
